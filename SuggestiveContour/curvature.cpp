...
//}
//
//void MeshCurvature::computePrincipalCurvature(HalfedgeMesh& m)
//{
//    max.resize(m.V.size());
//    min.resize(m.V.size());
//    max_direction.resize(m.V.size());
//    min_direction.resize(m.V.size());
//    tensor.resize(m.V.size(), Matrix3d::Zero());
//
//    for (int i = 0; i < m.V.size(); i++) {
//        int h_temp = m.h_out[i];
//        int h_end = h_temp;
...
//        // 領域内の辺を取得・領域内の面積を計算
//        do {
//            int v_id = m.hEList[h_temp].v_tgt(m.F);
//            int h_temp2 = m.h_out[v_id];
//            int h_end2 = h_temp2;
//
//            do {
//                // 探索している辺が内部辺であるとき、追加
//                if (m.hEList[h_temp2].h_opp >= 0) {
//                    auto key = std::make_pair(m.hEList[h_temp2].v_src(m.F), m.hEList[h_temp2].v_tgt(m.F));
//                    auto keyswap = std::make_pair(m.hEList[h_temp2].v_tgt(m.F), m.hEList[h_temp2].v_src(m.F));
//                    if (!edge_map.contains(key) && !edge_map.contains(keyswap)) {
//                        edge_map.emplace(key, edge_map.size());
//                        edges.push_back(h_temp2);
//                    }
...
//                // 探索している辺の h_next が内部辺であるとき、追加
//                if (m.hEList[ m.hEList[h_temp2].h_next() ].h_opp >= 0) {
//                    auto key = std::make_pair(m.hEList[m.hEList[h_temp2].h_next()].v_src(m.F), m.hEList[m.hEList[h_temp2].h_next()].v_tgt(m.F));
//                    auto keyswap = std::make_pair(m.hEList[m.hEList[h_temp2].h_next()].v_tgt(m.F), m.hEList[m.hEList[h_temp2].h_next()].v_src(m.F));
//                    if (!edge_map.contains(key) && !edge_map.contains(keyswap)) {
//                        edge_map.emplace(key, edge_map.size());
//                        edges.push_back(m.hEList[h_temp2].h_next());
//                    }
...
//        // テンソルを計算
//        for (auto& e : edges) {
//            Eigen::Vector3d edge = m.V[m.hEList[e].v_tgt(m.F)] - m.V[m.hEList[e].v_src(m.F)];
//            Eigen::Vector3d n = m.normalF[m.hEList[e].face()];
//            Eigen::Vector3d n_opp = m.normalF[ m.hEList[ m.hEList[e].h_opp ].face() ];
//
//            double angle_sign = (n.cross(n_opp)).dot(edge.normalized());
...
//
//        // 主曲率・主方向を計算
//        SelfAdjointEigenSolver<Matrix3d> ES(tensor[i]);
//        Eigen::Vector3d eval = ES.eigenvalues();
//        Matrix3d evec = ES.eigenvectors();
//        double normal_val = std::min({std::fabs(eval(0)), std::fabs(eval(1)), std::fabs(eval(2))});
//        if (normal_val == std::abs(eval(0))) {
//            max[i] = eval(2);
//            min[i] = eval(1);
...
//}
//
//void MeshCurvature::computeRadialCurvature(HalfedgeMesh const& m, Eigen::Vector3d const& viewpoint)
//{
//    // 1. radial curvature を計算
//    radial.resize(m.V.size());
//
...
//        // ∇λ(p0) = ( R(pi/2, normalF) * he(0)_vec ) / ( height(j) * |he(0)| )
//        for (int j = 0; j < 3; j++) {
//            grad_bc[j] = (AngleAxisd(std::numbers::pi, m.normalF[i]) * (m.V[m.F[i]((j+2)%3)] - m.V[m.F[i]((j+1)%3)])) / (2*area);
//        }
//
//        // ∇f = ∇λ(p0) * f(p0) + ∇λ(p1) * f(p1) + ∇λ(p2) * f(p2)
//        Eigen::Vector3d gradF = grad_bc[0] * radial[m.F[i](0)] + grad_bc[1] * radial[m.F[i](1)] + grad_bc[2] * radial[m.F[i](2)];
